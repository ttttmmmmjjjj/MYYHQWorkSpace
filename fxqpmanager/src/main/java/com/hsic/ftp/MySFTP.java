...
public class MySFTP {
	...
	public MySFTP(Context context) {
		...
		this.username = "fxlpg";
		this.password = "fxlpg2005";
		/**
		 * 根据公司设置用户名和密码
		 */
		companyInfo = context.getSharedPreferences("CompanyInfo", 0);// 公司
		Company=companyInfo.getString("CompanyName", "");
		if(Company.contains("SY")){
			this.username="shlpg";
			this.password="shlpg";;
		}
		if(Company.equals("CM")){
			this.username="cmlpg";
			this.password="cmlpg";;
		}
		if(Company.equals("HS")){
