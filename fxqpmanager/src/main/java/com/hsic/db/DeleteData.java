...
//            mDatabase.beginTransaction();
            ...
            sql="delete from T_B_CUSTOMERINFO where  date(InsertTime)< date('now','-2 day') and StationID=? and EmployeeID=? ";//巡检表
            mDatabase.execSQL(sql,args);
            ...
            sql="delete from T_B_XJ_Association where  date(InsertTime)< date('now','-2 day') and  EmployeeID=? ";//巡检附件关联表
