...
//			Log.e("=" + i, String.valueOf(ID[i]));
	 ...
	 * @throws IOException
	 */
	public String readOneBlockBySYDevice(int position) throws IOException {
	...
	public String readBlocks(int begin, int count) throws IOException{
		...
		for(int i = begin; i < count + begin; i++){
			data.append(readOneBlockByNormal(i));				
		}
	 ...
	 * @throws IOException
	 */
	public String ReadBlockBySYDevice(int begin, int count) throws IOException{
		if((begin + count) > blockNumber){
			count = blockNumber - begin;
		}
		StringBuffer data = new StringBuffer();	
		for(int i = begin; i < count + begin; i++){
			data.append(readOneBlockBySYDevice(i));
		}
		return data.toString(); 
	}
	
	// ��ͨ�豸��ȡ��ʽ
	public String readOneBlockByNormal(int position) throws IOException {
		byte cmd[] = new byte[11];
		cmd[0] = (byte) 0x22;
		cmd[1] = (byte) 0x20;
		System.arraycopy(ID, 0, cmd, 2, ID.length); // UID
	 ...
	 * @throws IOException
	 */
	public String ReadBlocksByNormal(int begin, int count) throws IOException {
		if ((begin + count) > blockNumber) {
			count = blockNumber - begin;
		}
		StringBuffer data = new StringBuffer();
