...
//    private ScrollView scrollView;
    ...
    private List<QPType> getItems() {
        return items;
    }

    public void setItems(List<QPType> list) {
        if (null == items) {
            items = new ArrayList<QPType>();
        }
        items.clear();
        items.addAll(list);
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
    ...
    private TextView createView(QPType item) {
        TextView tv = new TextView(context);
        tv.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        tv.setSingleLine(true);
        tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 20);
        tv.setText(item.getQPName());
...
//            position = divided + offset + 1;
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            String s=itemView.getText().toString();
            SpannableString msp = null;
            msp=new SpannableString(s);
            msp.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), 0, s.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            itemView.setText(msp);
            if (null == itemView) {
                return;
            }
            if (position == i) {
