...
public class GetEmpInfo_newTask extends AsyncTask<String, Void, HsicMessage>{
	private Context context;
	private ProgressDialog dialog;
	GetEmpInfo_newListener l;
	WebServiceCallHelper wb;
	GetBasicInfo getBasicInfo;//20181019
	public GetEmpInfo_newTask(Context context,GetEmpInfo_newListener l){
	...
	protected HsicMessage doInBackground(String... params) {
		...
		hsicMess=wb.GetEmpInfo_new( getBasicInfo.getDeviceID(), params[1], params[2]);
		return hsicMess;
	}
	...
	protected void onPostExecute(HsicMessage result) {
		...
		dialog.dismiss();
		l.GetEmpInfo_newListenerEnd(result);
	}
